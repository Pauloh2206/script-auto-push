...
# Script de Automação Git: Envio Completo Interativo V50 (FLUXO DE MENU CORRIGIDO)
...
function check_for_update() {
    ...
    if ! REMOTE_FILE=$(mktemp); then
        echo -e "${RED}❌ ERRO CRÍTICO: Não foi possível criar arquivo temporário. Verifique se o comando 'mktemp' está instalado (coreutils). Prosseguindo com V50.${NC}"
        return 1
    fi
    ...
    echo -e "${BLUE}🔎 Verificando por atualizações do próprio script (Timeout: 20s)...${NC}"
    ...
    if curl --max-time 20 -s "$REMOTE_SCRIPT_URL" > "$REMOTE_FILE"; then
        ...
        if [ -s "$REMOTE_FILE" ]; then 
            ...
            if cmp -s "$0" "$REMOTE_FILE"; then
                ...
                echo -e "${YELLOW}   Uma nova versão (V50+) do script foi detectada.${NC}"
                read -r -p "$(echo -e "${YELLOW}Deseja ATUALIZAR AGORA? (S/n): ${NC}")" UPDATE_CHOICE
                
                if [[ "$UPDATE_CHOICE" =~ ^[Ss]$ ]]; then
                    mv "$REMOTE_FILE" "$0"
                    chmod +x "$0"
                    echo -e "${GREEN}🚀 Script atualizado para a versão mais recente (V50).${NC}"
                    echo -e "${YELLOW}Por favor, RE-EXECUTE o script para aplicar as mudanças.${NC}"
                    trap - EXIT INT # Remove o trap para não tentar deletar o script que acabou de ser atualizado
                    exit 0
                else
                    echo -e "${YELLOW}⚠️ Atualização ignorada.${NC}"
                fi
            fi
            
        else
            echo -e "${RED}❌ ERRO DE ARQUIVO: O download falhou ou o arquivo remoto está vazio. Prosseguindo com V50.${NC}"
        fi
    else
        echo -e "${RED}❌ ERRO DE REDE: Não foi possível verificar atualizações (Timeout ou falha de conexão). Prosseguindo com V50.${NC}"
    fi
    
    # Remove o trap para não interferir na execução do script
    trap - EXIT INT
}
...
function main_menu() {
    ...
    while true; do
        echo -e "\n${YELLOW}=========================================================="
        echo -e "       MENU INICIAL - AUTOMAÇÃO GIT (V50)         "
        ...
        case "$MENU_CHOICE" in
            1)
                ...
                break # Sai do loop e continua para a inicialização do Git
                ;;
            2)
                check_for_update 
                echo -e "${GREEN}✅ Verificação concluída. Retornando ao menu para prosseguir.${NC}"
                # Sem 'break' aqui. O loop 'while true' repete, mostrando o menu novamente.
                ;; 
            3)
...
echo -e "\n${YELLOW}=========================================================="
echo -e "          INÍCIO DO ENVIO SIMPLIFICADO AO GITHUB (V50)          "
echo -e "${YELLOW}=========================================================="
echo -e "${NC}"
...
if [ -z "$REMOTE_URL" ]; then
    ...
    while true; do
        ...
        if [[ "$NEW_REPO_URL" =~ ^https://github.com/.*\.git$ ]]; then REMOTE_URL=$NEW_REPO_URL; break; fi
...
if ! git rev-parse --verify "$BRANCH_NAME" >/dev/null 2>&1; then
    ...
    if git commit -m "commit: Initial repository setup (Auto-generated by V50)" 2>/dev/null; then
    ...
    if git stash push -u -m "Auto-Stash antes do Pull Proativo V50" 2>/dev/null; then
        STASH_NEEDED=1
...
# ==========================================================
echo -e "\n${YELLOW}=========================================================="
echo -e "         FIM DO PROCESSO GIT INTERATIVO (V50)         "
echo -e "=========================================================="
