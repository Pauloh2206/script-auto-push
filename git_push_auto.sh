...
# Script de Automação Git: Envio Completo Interativo V45 (MENU INICIAL)
...
function check_for_update() {
    ...
    if curl --max-time 10 -s "$REMOTE_SCRIPT_URL" > /tmp/git_push_auto_remote.sh; then
        ...
        if cmp -s "$0" /tmp/git_push_auto_remote.sh; then
            ...
            echo -e "${YELLOW}   Uma nova versão (V45+) do script foi detectada.${NC}"
            ...
            if [[ "$UPDATE_CHOICE" =~ ^[Ss]$ ]]; then
                ...
                chmod +x "$0"
                echo -e "${GREEN}🚀 Script atualizado para a versão mais recente (V45).${NC}"
...
function main_menu() {
    echo -e "\n${YELLOW}=========================================================="
    echo -e "       MENU INICIAL - AUTOMAÇÃO GIT (V45)         "
    echo -e "      ${CYAN}Autor: Paulo Hernani | Assistência: Gemini${NC}"
    echo -e "${YELLOW}=========================================================="
    
    while true; do
        echo -e "${CYAN}Escolha uma opção:${NC}"
        echo -e "1) ${GREEN}INICIAR PUSH/SINCRONIZAÇÃO${NC} (Pula verificação de atualização)"
        ...
        case "$MENU_CHOICE" in
            ...
            2)
                check_for_update 
                # Se o check_for_update sair (atualizou), o script para. Se não, continua.
                echo -e "${GREEN}✅ Verificação concluída. Prosseguindo com o script...${NC}"
                break 
                ;;
            3)
...
# ==========================================================
check_dependencies
main_menu # <-- NOVO MENU INTERATIVO

echo -e "\n${YELLOW}=========================================================="
echo -e "          INÍCIO DO ENVIO SIMPLIFICADO AO GITHUB (V45)          "
echo -e "${YELLOW}=========================================================="
echo -e "${NC}"
...
if ! git rev-parse --verify "$BRANCH_NAME" >/dev/null 2>&1; then
    ...
    if git commit -m "commit: Initial repository setup (Auto-generated by V45)" 2>/dev/null; then
    ...
    if git stash push -u -m "Auto-Stash antes do Pull Proativo V45" 2>/dev/null; then
        STASH_NEEDED=1
...
# ==========================================================
echo -e "\n${YELLOW}=========================================================="
echo -e "         FIM DO PROCESSO GIT INTERATIVO (V45)         "
echo -e "=========================================================="
